...
1. 上传 `生产计划` 目录一份 ≤50MB 的 `.xlsx` → **预期**：201，返回文件名带
   `_YYYYMMDD_HHMMSS` 时间戳。
2. 越界上传：`dir=tmp` → 403；上传 `.exe` 或 >50MB → 400/413。
...
5. 让数字人"把分析结果写入临时空间" → 在 `tmp/` 出现 `{thread_id}_*_结果.csv`；
   `GET /api/files/list?dir=tmp` 可见并可下载。
