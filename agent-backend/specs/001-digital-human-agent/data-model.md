...
**校验/规则**：删除 thread 连带删除 history/summary 及 `tmp/` 下该
...
| 属性                                                                 | 说明                                                                                            |
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 目录类别                                                             | 7 个业务目录（Agent 只读）、`shared/`（Agent 只读，用户可传）、`tmp/`（Agent 可写，用户不可传） |
| 命名                                                                 | 用户上传追加 `_YYYYMMDD_HHMMSS`；tmp 文件强制 `{thread_id}_{timestamp}_{filename}`              |
| 限制                                                                 | 单文件 ≤50MB；扩展名 ∈ {.csv,.xlsx,.txt,.json,.pdf}                                             |
| 清理                                                                 | tmp 中 7 天未访问自动清除（**访问时间实现**：read_file 读到 tmp 文件时用                        |
| `fs.utimes` 手动刷新 mtime——atime 在 relatime/noatime 挂载下不可靠； |
| 清理按该时间判断）；删 thread 连带清除其前缀文件                     |

**权限矩阵**（FR-022，由 file-access 代理层强制）：

| 目录       | Agent 读 |        Agent 写         | 用户上传 |
| ---------- | :------: | :---------------------: | :------: |
| 7 业务目录 |    ✅    |           ❌            |    ✅    |
| shared/    |    ✅    |           ❌            |    ✅    |
| tmp/       |    ✅    | ✅（须 thread_id 前缀） |    ❌    |
