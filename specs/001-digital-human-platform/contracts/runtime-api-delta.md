...
### 10.7 服务侧契约（`ocr-service`，2026-09-25）

**范围**：本特性在服务侧**只改 `ocr-service` 的 `ocr_image` 一个工具**。`jev-service` 与任何第三方 MCP 服务**一律不动**——异步是**按工具声明**的能力，未声明的服务/工具行为零变化。
...
| `result_url` | 行为 |
|---|---|
| 缺省 / 空 | **同步**（既有行为，一字未改）：校验 → 下载 → 识别 → 返回文本 |
| 有值且通过 host 白名单 | **异步**：**立即返回受理** `{"job_id":"…","status":"accepted","message":"…"}`；识别在后台线程进行，完成后把结果 `POST` 回 `result_url` |

**回写形状**：`POST {result_url}&filename={job_id}.txt`，body = 识别文本（utf-8，`content-type: text/plain; charset=utf-8`）。`job_id` 形如 `ocr_<毫秒时间戳>_<随机 8 位>`（时间戳在前便于人眼排序，随机后缀防同毫秒碰撞）。落盘名由运行环境补 `{prefix}_` 前缀 ⇒ `{prefix}_ocr_….txt`（§10.3/§10.4 口径）。
...
**可测性**：受理响应构造、回写 URL 拼装（保留原有 query）、`job_id` 生成与 `POST` 回写都放在 `ocr_core`（**不依赖模型**），单测在宿主机本地跑（原则三）。
